...
int main(int argc, char *argv[])
{
	...
	for(i=0;i<5;i++){
		...
		while(terminar ==0)
		{
			...
			//VAmos a crear un arreglo para evitar errores en el formato del nombre
			if(codigo !=0)
			{
			...
			else if (codigo ==1) //piden la longitd del nombre (Primer servicio)
				sprintf (respuesta,"%d",strlen (nombre));//La va a escribir en un string  y lo va a colocar en respuesta
			...
			//VAmos a hacer un arreglo para cuando la repsuesta no sea el formato que esperamos y evitar errores
			if (codigo !=0)
			{
				printf ("Respuesta: %s\n", respuesta);
				// Y lo enviamos
